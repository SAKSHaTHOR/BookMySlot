...
class AppointmentController extends GetxController{
   ...
   // var ElecName = "".obs;
   bookAppointment(String ElecId,String ElecName,context) async{
      isLoading(true);
   ...
   Future<QuerySnapshot<Map<String,dynamic>>> getAppointments(bool isElectrician){
      if(isElectrician) {
         return FirebaseFirestore.instance.collection('appointments').where(
             'appWith', isEqualTo: FirebaseAuth.instance.currentUser?.uid).get();
      }else{
         return FirebaseFirestore.instance.collection('appointments').where(
             'appBy', isEqualTo: FirebaseAuth.instance.currentUser?.uid).get();
      }
      }
}